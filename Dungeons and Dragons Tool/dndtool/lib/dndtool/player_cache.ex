...
defmodule DnDTool.Player_Cache do
  ...
  def load_players() do
    ...
    { :ok, save_text } = File.read("../dndtool/save/players.txt")
  ...
  def save_players(cache) do
    ...
    File.write("../dndtool/save/players.txt", save_text)

  end

  def get_player(cache, key) do
    { :ok, player } = Map.fetch(cache, key)
    player
  end
  ...
  defp player_to_string(player_map) do
   ...
   "name:" <> Map.get(player_map, :name) <> "," <> map_text
